...
def lyrics():
	...
	os.system("""sudo ./scrolling-text-example --led-cols=64 -y 8 -l 1 -s 10 -f ../fonts/9x18B.bdf -C """ + setColor + lyric)
...
def demo():
	...
	try:
		if chosen == "0":
			...
			proc = subprocess.run(cmd, timeout=300, shell=True)
		elif chosen == "8":
			cmd = cmd + "--led-brightness=70 "
			proc = subprocess.run(cmd, timeout=120, shell=True)
		elif chosen == "9":
		...
		pgid= os.getpgid(os.getpid())
		if pgid == 1:
			os.kill(os.getput(), signal.CTRL_C_EVENT)
		else:
			os.killpg(os.getpgid(os.getpid()), signal.SIGINT)
	finally:
		main()
