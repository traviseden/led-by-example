...
def lyrics():
	...
	fontSet = fontSettings()
	#os.system("""sudo ./scrolling-text-example --led-cols=64 -s 13 -y 8 -l 1 -f ../fonts/9x18B.bdf -C """ + setColor + lyric)
...
def playMovie():
	movie = movieSettings()
	utilFolder()
	cmd = "sudo ./video-viewer --led-cols=64 -V30 -f " + movie
	try:
...
def getPrice():
	...
	apiFolder()
	fontSet = fontSettings()
	#cmd = "sudo ./scrolling-text-example --led-cols=64 -s 12 -y 8 -l 2 -f ../fonts/9x18B.bdf -C " + setColor + "Price: " + price
...
def displayTime():
	...
	setColor2 = ran_color()
	apiFolder()
	cmd = "sudo ./clock --led-cols=64 -f ../fonts/6x12.bdf -x 8 -d %I:%M:%S -d %p -O " + setColor + " -C " + setColor2
	try:
		proc = subprocess.run(cmd, timeout=15, shell=True)
	except subprocess.TimeoutExpired:
		pgid = os.getpgid(os.getpid())
...
def randomize(cycleLimit, btcLimit):
	...
	cmd = ['lyrics()']
	#cmd = ['demo()', 'playImage()', 'quotes()', 'lyrics()', 'getPrice()', 'playMovie()', 'displayTime()']
	if cycleLimit > 0:
